...
public class Chessboard extends JPanel {
	...
	private void setFigures4NewGame(int i, Player player, boolean upsideDown) {
		...
		if (i != 0 && i != 7) {
			...
			LogToFile.log(null, "Debug", "error setting figures like rook etc.");
			return;
		} else if (i == 0) {
	...
	public void select(Square sq) {
		...
		LogToFile.log(null, "INFO", "active_x: " + this.active_x_square + " active_y: " + this.active_y_square);
		repaint();
	...
	public void move(int xFrom, int yFrom, int xTo, int yTo) {
		...
		try {
			...
			LogToFile.log(exc, "Error", "error moving piece: ");
			return;
		}
	...
	public void paintComponent(Graphics g) {
																		...
																		// active
		{
			...
			for (Iterator it = moves.iterator(); moves != null && it.hasNext();) {
				...
				g2d.drawImage(able_square, (sq.getPozX() * (int) square_height) + topLeftPoint.x,
