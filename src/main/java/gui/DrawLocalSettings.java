...
public class DrawLocalSettings extends JPanel implements ActionListener, TextListener {
	...
	JCheckBox timeGame;
	JComboBox time4Game;
	String colors[] = { Settings.lang("White"), Settings.lang("Black"), Settings.lang("Blue"), Settings.lang("Red"),
			Settings.lang("Green") };
	...
	public void actionPerformed(ActionEvent e) {
		...
		} else if (target == this.okButton) // if clicked OK button (on finish)
		{
			...
			if (!this.oponentComp.isSelected() && (this.firstName.getText().length() == 0
					|| this.secondName.getText().length() == 0 || this.thirdName.getText().length() == 0)) {
			...
			if (this.oponentComp.isSelected()) // if computer opponent is
												// checked
			{
			 ...
			 * sett.upsideDown = true; } else { sett.upsideDown = false; } if
			 * (this.timeGame.isSelected()) // if timeGame is checked { String
			 * value = this.times[this.time4Game.getSelectedIndex()];// set //
			 * time // for // game Integer val = new Integer(value);
			 * sett.timeLimitSet = true; sett.timeForGame = (int) val * 60;//
			 * set time for game and mult // it to seconds
			 * newGUI.getGameClock().setTimes(sett.timeForGame,
			 * sett.timeForGame); newGUI.getGameClock().start(); }
			 */
	...
	DrawLocalSettings(JDialog parent) {
		 ...
		 * (thirdName.isEnabled()==true){ thirdName.setEnabled(false); }else {
